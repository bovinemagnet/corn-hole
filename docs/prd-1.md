...
single-player feel and loop

LAN host-mode multiplayer + join codes

authoritative shared-world consumption

battle royale (hole-vs-hole)

late join/reconnect hardening

dedicated server deployment (home server; later remote)

expansion to desktop and eventually console platforms like Nintendo Switch and PlayStation 5 (subject to platform programs and certification)
...
Fast private matchmaking: create match → share join code → play

Responsive controls: touch movement feels “arcade tight”

Shared world: consumed objects disappear for all players

Scalable to 20 players by avoiding syncing thousands of physics objects

Battle royale mode: holes can eat holes and inherit mass

Robust sessions: late join and reconnect supported (eventually)

Future-ready: architecture supports dedicated servers and remote play
...
LAN party: Kids at home, one hosts, others join via code

School friends remote (future): join code shared via message, dedicated server hosts

Battle royale: last hole standing; kills transfer mass
...
closest distance to object center

if tie within epsilon: larger radius wins

if still tie: stable tie-break (lowest playerId)
