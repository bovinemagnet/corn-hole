...
# "Cute rounded": bevel + smooth shading + optional subdiv
USE_SUBDIV = True
SUBDIV_LEVEL = 1

# Layout grid for review in Blender
GRID_COLS = 10
GRID_SPACING = 2.6
...
def make_material(name, rgb, rough=0.85, spec=0.15):
    ...
    # Helper: set input if it exists on this Blender version
    ...
    # Roughness exists in all recent versions
    set_in("Roughness", rough)

    # Specular changed in Blender 4.x:
    # - Blender 3.x: "Specular"
    # - Blender 4.x: "Specular IOR Level"
    if not set_in("Specular", spec):
        set_in("Specular IOR Level", spec)

    # Optional: keep things cartoony (low shine)
    set_in("Metallic", 0.0)
...
def make_materialv3(name, rgb, rough=0.85, spec=0.15):
    mat = bpy.data.materials.new(name=name)
    mat.use_nodes = True
...
def add_stickers(base_parts, count, area_min=(0.18, 0.12), area_max=(0.28, 0.18), thickness=0.02):
    """
    Add small sticker cubes (raised squares) to the *top* faces of toys.
    base_parts: list of objects that represent the main visible surfaces.
    Returns list of sticker objects.
    """
    stickers = []
    ...
    for i in range(count):
        ...
        # Place sticker near top of first base part
        target = random.choice(base_parts)
        # Approx bounds from target scale/location (cheap but fine for our simple models)
        x = random.uniform(-0.20, 0.20)
        y = random.uniform(-0.15, 0.15)
        z = 0.0

        # Put sticker slightly above the expected "top" of the target
        # For our toys, tops are usually around z~0.2–0.4
...
def add_leaf_clumps(bush_obj, variant_index):
    """
    Add 1–3 leaf clumps as extra spheres, more on higher variants.
    """
    clumps = []
    base_count = 1 + variant_index  # v0=1, v1=2, v2=3
...
def make_sign_shape(shape_kind, mat_white, mat_bark):
    """
    Returns (post, sign) objects.
    shape_kind: 'rect', 'rounded', 'arrow'
    """
    post = rounded_cube("tmp_post", size=1.0, scale=(0.10, 0.10, jitter(1.2, 0.06)), bevel=0.03, mat=mat_bark)

    if shape_kind == "rect":
        sign = rounded_cube("tmp_sign_rect", size=1.0, scale=(0.85, 0.18, 0.55), bevel=0.06, mat=mat_white)
    elif shape_kind == "rounded":
        sign = rounded_cube("tmp_sign_round", size=1.0, scale=(0.80, 0.18, 0.50), bevel=0.12, mat=mat_white)
    else:
        # Arrow: rectangle + small arrow head
        body = rounded_cube("tmp_arrow_body", size=1.0, scale=(0.75, 0.18, 0.40), bevel=0.08, mat=mat_white)
        head = rounded_cube("tmp_arrow_head", size=1.0, scale=(0.30, 0.18, 0.30), bevel=0.08, mat=mat_white)
        head.location = (0.55, 0.0, 0.05)
        sign = join([body, head], "tmp_sign_arrow")

    sign.location = (0.0, 0.0, 1.0)
    return post, sign

# =========================
# PROP FACTORY (variant-aware, with visual differences)
# =========================
def make_prop(prop_key, variant_index):
    scale_jitter = 0.07 if variant_index > 0 else 0.0
    roughen_jitter = 0.03 * variant_index

    # Stickers apply mostly to toy props
    sticker_count = 0
    if prop_key in ("PROP_ToyCar", "PROP_Scooter", "PROP_Ball", "PROP_Frisbee", "PROP_Bucket"):
        sticker_count = variant_index  # v0=0, v1=1, v2=2 stickers

    if prop_key == "PROP_Bush":
        m = pick_mat("grass", "green2")
        base = simple_sphere("tmp_bush", radius=jitter(0.65, 0.10), mat=m, roughen=0.10+roughen_jitter)
        base.scale = jitter_vec3((1.2, 1.0, 0.9), scale_jitter)
        apply_modifiers(base)
        # Add extra clumps to make variants obvious
        clumps = add_leaf_clumps(base, variant_index)
        obj = join([base] + clumps, "PROP_Bush")
        obj.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(obj)
        return obj

    if prop_key == "PROP_RockSmall":
        s = simple_sphere("PROP_RockSmall", radius=jitter(0.35, 0.10), mat=PALETTE["rock"], roughen=0.18+roughen_jitter)
        s.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(s)
        return s

    if prop_key == "PROP_RockLarge":
        s = simple_sphere("PROP_RockLarge", radius=jitter(0.75, 0.10), mat=PALETTE["rock"], roughen=0.20+roughen_jitter)
        s.scale = jitter_vec3((1.1, 1.0, 0.9), scale_jitter)
        apply_modifiers(s)
        return s

    if prop_key == "PROP_Log":
        log = capsule("PROP_Log", radius=jitter(0.22, 0.08), length=jitter(1.6, 0.10), mat=PALETTE["wood"])
        log.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(log)
        return log
    ...
    if prop_key == "PROP_SwingSet":
        ...
        seat = rounded_cube("tmp_swing", size=1.0, scale=jitter_vec3((0.35, 0.22, 0.06), 0.10), bevel=0.04, mat=seat_mat)
    ...
    if prop_key == "PROP_SignPost":
        # Different sign shapes per variant
        shape = ["rect", "rounded", "arrow"][variant_index]
        post, sign = make_sign_shape(shape, PALETTE["white"], PALETTE["bark"])
        obj = join([post, sign], "PROP_SignPost")
        obj.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(obj)
        return obj
    ...
    if prop_key == "PROP_Ball":
        ...
        s = simple_sphere("PROP_Ball", radius=jitter(0.35, 0.08), mat=PALETTE[m], roughen=0.02)
        # stickers on ball (tiny raised squares)
    ...
    if prop_key == "PROP_Fountain":
        ...
        bowl = simple_sphere("tmp_fbowl", radius=jitter(0.55, 0.06), mat=PALETTE["blue"], roughen=0.03)
    ...
    if prop_key == "PROP_Mushroom":
        ...
        cap  = simple_sphere("tmp_mcap", radius=jitter(0.28, 0.08), mat=PALETTE[cap_m], roughen=0.03)
        cap.location.z += 0.35
    ...
    if prop_key == "PROP_LeafPile":
        ...
        pile = simple_sphere("tmp_leafpile", radius=jitter(0.40, 0.10), mat=PALETTE[m], roughen=0.10+roughen_jitter)
    ...
    if prop_key == "PROP_Acorn":
        nut = simple_sphere("tmp_acorn", radius=jitter(0.18, 0.08), mat=PALETTE["brown"], roughen=0.03)
    ...
    if prop_key == "PROP_PineCone":
        cone = simple_sphere("PROP_PineCone", radius=jitter(0.22, 0.08), mat=PALETTE["bark"], roughen=0.12+roughen_jitter)
    ...
    if prop_key == "PROP_TreeSmall":
        ...
        crown = simple_sphere("tmp_crownS", radius=jitter(0.55, 0.10), mat=pick_mat("grass", "green2"), roughen=0.06+roughen_jitter)
    ...
    if prop_key == "PROP_TreeBig":
        ...
        crown = simple_sphere("tmp_crownB", radius=jitter(0.85, 0.10), mat=pick_mat("grass", "green2"), roughen=0.07+roughen_jitter)
    ...
    if prop_key == "PROP_Hedge":
        ...
        hedge = rounded_cube("PROP_Hedge", size=1.0, scale=jitter_vec3((1.3, 0.45, 0.55), 0.08), bevel=0.10, mat=m)
        return hedge

    if prop_key == "PROP_PicnicBlanket":
        m = ["red", "blue", "yellow"][variant_index]
        blanket = rounded_cube("PROP_PicnicBlanket", size=1.0, scale=jitter_vec3((1.4, 1.0, 0.06), 0.08), bevel=0.06, mat=PALETTE[m])
        return blanket

    if prop_key == "PROP_PathMarker":
        marker = rounded_cube("PROP_PathMarker", size=1.0, scale=jitter_vec3((0.22, 0.22, 0.55), 0.08), bevel=0.08, mat=PALETTE["white"])
        return marker
