...
def make_prop(prop_key, variant_index):
    # Variant rules:
    # - slight scale jitter
    # - slight roughen change for organic shapes
    # - slight material swap within same family
    ...
    if prop_key == "PROP_SwingSet":
        metal = PALETTE["metal"]
        seat_mat = pick_mat("blue", "red", "yellow")
        legL = rounded_cube("tmp_legL", size=1.0, scale=(0.12, 0.12, jitter(1.6, 0.05)), bevel=0.05, mat=metal)
        legR = rounded_cube("tmp_legR", size=1.0, scale=(0.12, 0.12, jitter(1.6, 0.05)), bevel=0.05, mat=metal)
        legL.location = (-0.7, 0.0, 0.0)
        legR.location = ( 0.7, 0.0, 0.0)
        bar  = rounded_cube("tmp_bar", size=1.0, scale=(jitter(1.7, 0.05), 0.10, 0.10), bevel=0.04, mat=metal)
        bar.location.z += 1.6
        seat = rounded_cube("tmp_swing", size=1.0, scale=jitter_vec3((0.35, 0.22, 0.06), 0.08), bevel=0.04, mat=seat_mat)
    ...
    if prop_key == "PROP_SignPost":
        ...
        sign = rounded_cube("tmp_sign", size=1.0, scale=jitter_vec3((0.85, 0.18, 0.55), 0.06), bevel=0.08, mat=PALETTE["white"])
    ...
    if prop_key == "PROP_Ball":
        m = pick_mat("orange", "blue", "red")
        s = simple_sphere("PROP_Ball", radius=jitter(0.35, 0.08), mat=m, roughen=0.02)
        s.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(s)
        return s

    if prop_key == "PROP_Frisbee":
        m = pick_mat("red", "blue", "yellow")
        disc = rounded_cube("PROP_Frisbee", size=1.0, scale=jitter_vec3((0.55, 0.55, 0.08), 0.08), bevel=0.06, mat=m)
        return disc

    if prop_key == "PROP_Bucket":
        m = pick_mat("yellow", "blue", "red")
        body = rounded_cube("PROP_Bucket", size=1.0, scale=jitter_vec3((0.45, 0.45, 0.50), 0.08), bevel=0.10, mat=m)
        return body

    if prop_key == "PROP_GardenGnome":
        body_m = pick_mat("red", "blue", "yellow")
        body = simple_sphere("tmp_gnome_body", radius=jitter(0.35, 0.08), mat=body_m, roughen=0.04)
        head = simple_sphere("tmp_gnome_head", radius=jitter(0.22, 0.06), mat=PALETTE["white"], roughen=0.03)
        head.location = (0.0, 0.0, 0.50)
        obj = join([body, head], "PROP_GardenGnome")
        obj.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(obj)
        return obj
    ...
    if prop_key == "PROP_BirdHouse":
        ...
        roof_m = pick_mat("red", "blue", "yellow")
        roof = rounded_cube("tmp_bh_roof", size=1.0, scale=jitter_vec3((0.65, 0.65, 0.20), 0.08), bevel=0.08, mat=roof_m)
    ...
    if prop_key == "PROP_ParkBin":
        m = pick_mat("green2", "blue", "red")
        return rounded_cube("PROP_ParkBin", size=1.0, scale=jitter_vec3((0.60, 0.60, 0.80), 0.08), bevel=0.10, mat=m)
    ...
    if prop_key == "PROP_Mushroom":
        ...
        cap_m = pick_mat("red", "orange", "blue")
        cap  = simple_sphere("tmp_mcap", radius=jitter(0.28, 0.08), mat=cap_m, roughen=0.03)
        cap.location.z += 0.35
    ...
    if prop_key == "PROP_LeafPile":
        m = pick_mat("orange", "yellow", "red")
        pile = simple_sphere("PROP_LeafPile", radius=jitter(0.40, 0.10), mat=m, roughen=0.10+roughen_jitter)
    ...
    if prop_key == "PROP_ToyCar":
        body_m = pick_mat("red", "blue", "yellow")
        body = rounded_cube("tmp_carbody", size=1.0, scale=jitter_vec3((0.70, 0.40, 0.20), 0.08), bevel=0.08, mat=body_m)
        ...
        obj = join([body, top], "PROP_ToyCar")
        obj.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(obj)
        return obj

    if prop_key == "PROP_Scooter":
        deck_m = pick_mat("blue", "red", "yellow")
        deck = rounded_cube("tmp_deck", size=1.0, scale=jitter_vec3((0.85, 0.22, 0.08), 0.08), bevel=0.06, mat=deck_m)
        ...
        obj = join([deck, handle, bar], "PROP_Scooter")
        obj.scale = jitter_vec3((1.0, 1.0, 1.0), scale_jitter)
        apply_modifiers(obj)
        return obj
    ...
    if prop_key == "PROP_Sandbox":
        m = pick_mat("yellow", "blue", "red")
        return rounded_cube("PROP_Sandbox", size=1.0, scale=jitter_vec3((1.2, 1.2, 0.25), 0.08), bevel=0.12, mat=m)
    ...
    if prop_key == "PROP_PicnicBlanket":
        m = pick_mat("red", "blue", "yellow")
        blanket = rounded_cube("PROP_PicnicBlanket", size=1.0, scale=jitter_vec3((1.4, 1.0, 0.06), 0.08), bevel=0.06, mat=m)
        return blanket
    ...
    if prop_key == "PROP_Stroller":
        m = pick_mat("blue", "red", "yellow")
        body = rounded_cube("tmp_stroll", size=1.0, scale=jitter_vec3((0.65, 0.45, 0.25), 0.08), bevel=0.10, mat=m)
